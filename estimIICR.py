...
def compute_IICR_general(t,params):
    ...
    if "tau" in params:
        tau = params["tau"]
        c = params["size"]
        return compute_piecewise_stationary_IICR_general(t, params)
...
def compute_piecewise_stationary_IICR_general(t, params):
    ...
    model_params = {"nbLoci" : 100, "samplingVector" : s, "scenario" : scenarios}
...
if __name__ == "__main__":
    ...
    for i in range(len(p["scenarios"])):
        ...
        #obs = 3*np.array(obs) #FacSexCoalescent
        ...
        #mean_t = sum(obs)/len(obs)
        #print("Moyenne : " + str(mean_t))
        #var_t = (1/len(obs))*sum((obs-mean_t)**2)
        #print("Variance : " + str(var_t))
        #with open("./stats_{}_file.txt".format(i), "w") as f:
                    #f.write("{}\n".format(ms_full_cmd))
                    #f.write("Moyenne {}\n".format(mean_t))
                    #f.write("Variance {}\n".format(var_t))
        #fig = plt.figure()
        #ax = fig.add_subplot(1,1,1)
        #ax.hist(obs,bins='auto')
        #path_hist = "hist_" +str(i) + ".png"
        ...
        #sample_size = int(ms_command.split("FacSexCoalescent ")[1].split(" ")[1])+1
        #sample_size = 2 #FacSexCoalescent
        factor = comb(sample_size, 2)
        print(sample_size,factor)
